...
<section class="card" style="margin-top:16px;"><div class="section-title"><h2>Payroll Report Preview</h2><div>@if ($isSuperAdminViewing ?? false)<span class="badge badge-amber" title="Hanya tersedia untuk Tim HR/Finance">View Only</span>@else<button class="btn btn-secondary">@include('payflow.partials.icon', ['name' => 'download', 'class' => 'icon icon-sm']) Export PDF</button><button class="btn btn-secondary">@include('payflow.partials.icon', ['name' => 'download', 'class' => 'icon icon-sm']) Export Excel</button>@endif</div></div>@include('payflow.pages.parts.payroll-table')</section>
